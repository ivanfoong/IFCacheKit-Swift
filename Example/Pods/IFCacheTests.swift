...
class IFCacheSpec: QuickSpec {
    ...
    override func spec() {
        describe("the cache") {
            it("can put items") {
                var cache = IFCache<Author, Book>(cacheDirectoryPath: self.cacheDirectoryPath, lruItemSize: 1)
                let authors = self.sampleAuthors()
                let books = self.sampleBooks()
                
                cache.put(authors[0], value: books[0])
                expect(cache.get(Set([authors[0]]))).toNot(beNil())
                
                cache.put(authors[1], value: books[1])
                expect(cache.get(Set([authors[1]]))).toNot(beNil())
            }
